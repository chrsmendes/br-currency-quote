...
export default class UIController {
  ...
  static _createCurrencyListItem(currency, onItemSelected) {
    ...
    listItem.addEventListener("click", function () {
      ...
      const selectedDropdown =
        dropdownContainer.querySelector(".selected-dropdown");
   ...
   * @private
   */
  static _updateConversionCurrencyOptions(
    selectedDropdown,
    currencies,
    ...
    if (selectedCurrency === "BRL") {
      ...
      const conversionDropdown = conversionCurrencyUl
        .closest(".dropdown")
        .querySelector(".selected-dropdown");
  ...
  static _setOnlyBRLAsConversionOption(currencies, conversionCurrencyUl) {
    const brlCurrency = currencies.find(
      (currency) => currency.simbolo === "BRL",
    );
    if (brlCurrency) {
  ...
  static populateCurrencyOptions(currencies, currencyUl, conversionCurrencyUl) {
    ...
    currencies.forEach((currency) => {
      const listItem = this._createCurrencyListItem(
        currency,
        (selectedDropdown) =>
          this._updateConversionCurrencyOptions(
            selectedDropdown,
            currencies,
            conversionCurrencyUl,
          ),
  ...
  static _validateCurrencySelection(sourceCurrency, targetCurrency) {
    if (sourceCurrency !== "BRL" && targetCurrency !== "BRL") {
      this.showNotification(
        "Conversions are only allowed between BRL and other currencies.",
        "warning",
      );
      return false;
    }
   ...
   * @private
   */
  static async _fetchExchangeRateData(
    selectedCurrency,
    conversionCurrency,
    selectedDate,
    amount,
    ...
    if (selectedCurrency === "BRL") {
      ...
      data = await CurrencyConversion.getBRLExchangeRate(
        conversionCurrency,
        selectedDate,
      );
    } else {
      // Use CurrencyService for foreign currency to BRL
      data = await CurrencyService.getExchangeRate(
        selectedCurrency,
        selectedDate,
      );
    }
  ...
  static async handleFormSubmission(
    ...
    form.addEventListener("submit", async (event) => {
      ...
      const conversionCurrency =
        conversionCurrencySpan.getAttribute("data-value");
      const amount = parseFloat(document.getElementById("amount").value);
      const selectedDate = document.getElementById("date").value;

      // Validate that one of the currencies is BRL
      if (
        !this._validateCurrencySelection(selectedCurrency, conversionCurrency)
      ) {
        return;
      }
      ...
      try {
        ...
        const lastRate =
          calculatedData.cotacoes && calculatedData.cotacoes.length > 0
            ? calculatedData.cotacoes[calculatedData.cotacoes.length - 1]
            : null;

        if (!lastRate) {
          throw new Error(
            "No exchange rate data available for the selected date.",
          );
        }
    ...
    if (shareLinkButton) {
      shareLinkButton.addEventListener("click", () => {
        ...
        navigator.clipboard
          ...
          .catch(() => {
            UIController.showNotification(
              `Failed to copy the link. Here it is: ${shareableLink}`,
              "danger",
            );
          });
      });
