...
class CurrencyService {
  ...
  static async getCurrencies() {
    ...
    const [response, flagsData] = await Promise.all([
      fetch(url),
      this.fetchFlags(),
    ]);
    if (!response.ok) {
    ...
    const currenciesWithFlags = data.map((currency) => {
      ...
      return {
        ...
        flag:
          flagInfo?.flag ??
          "https://files.softicons.com/download/internet-cons/flag-icons-by-custom-icon-design/png/32/European-Union-Flag.png",
      };
    });
  ...
  static async getExchangeRate(currency, date) {
    ...
    if (!response.ok) {
      ...
      const errorMessage =
        errorData.message ||
        "An unknown error occurred while fetching the exchange rate.";
